...
<%@ page
	import="java.util.*, com.foodexpress.updatedDTO.MenuItems, com.foodexpress.updatedDTO.OrderItem ,com.foodexpress.updatedDTO.Restaurant"%>
<!DOCTYPE html>
<html>
<head>
<title>Menu</title>
...
<body>
	...
	<%
	if (restaurant != null) {
		System.out.println("Restaurant id from menu.jsp page: " + restaurant.getRestaurantId());
	%>
			...
			<div class="restaurant-meta">
				...
				<div class="meta-item">
					...
					<div>
						...
						%>
						<span class="rating-badge"> <i class="fas fa-star"></i> <%=restaurant.getRating()%>
						</span>
						<%
						} else {
						%>
				...
				<div class="meta-item">
					...
					<div>
						<span class="status-badge <%=statusClass%>"> <i
							class="<%=statusIcon%>"></i> <%=restaurant.getStatus() != null ? restaurant.getStatus() : "N/A"%>
						</span>
					</div>
				</div>
			</div>
	...
	<div class="menu-container">
		...
		<div class="menu-grid" id="menu-grid">
			...
			for (MenuItems item : menuItems) {
				...
				? item.getImageUrl()
				: "https://thumbs.dreamstime.com/b/classic-mexican-taco-meat-fresh-vegetables-vector-food-design-generative-ai-simple-clean-illustration-filled-lettuce-394666411.jpg";
				String itemType = item.isVeg() ? "veg" : "nonveg";
			%>
			<div class="menu-card" data-type="<%=itemType%>"
				data-id="<%=item.getItemId()%>">
				<img src="<%=itemImage%>"
					alt="https://thumbs.dreamstime.com/b/classic-mexican-taco-meat-fresh-vegetables-vector-food-design-generative-ai-simple-clean-illustration-filled-lettuce-394666411.jpg"
					...
					<div class="item-footer">
						...
						<div class="action-area">
							...
							<button type="button" class="add-btn disabled"
								style="background: grey; cursor: not-allowed;" disabled>
							...
							<div class="qty-control" data-id="<%=item.getItemId()%>"
								data-restaurant="<%=restaurant.getRestaurantId()%>"
								data-price="<%=item.getPrice()%>">
								<button class="qty-btn minus">-</button>
								<span class="qty-count"><%=existingQty%></span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // AJAX part
    function updateCart(itemId, restaurantId, quantity, price) {
        fetch("<%=request.getContextPath()%>/CartServlet", {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            },
            body: `itemId=${itemId}&restaurantId=${restaurantId}&quantity=${quantity}&price=${price}`
        })
        .then(res => {
        if (res.status === 401) {
        	showLoginAlert();
        	...
        	// After 1.5 sec
        	setTimeout(() => {
                window.location.href = "<%=request.getContextPath()%>/DisplayRestaurant";
            }, 1500);
            return;
        }
        return res.json();
    ...
    function showLoginAlert() {
        ...
        alertDiv.role = "alert";
        alertDiv.innerHTML = `
            <strong>Please login</strong> to continue adding items.
    ...
    document.getElementById('menu-grid').addEventListener('click', function(e) {
    	if (e.target.closest('.add-btn')) {
    	    ...
    	    const actionArea = e.target.closest('.action-area');
    	    actionArea.innerHTML = `
    	        <div class="qty-control" data-id="${itemId}" data-restaurant="${restaurantId}" data-price="${price}">
    	            <button class="qty-btn minus">-</button>
    	            <span class="qty-count">1</span>
    	            <button class="qty-btn plus">+</button>
    	        </div>
    	    `;
    	    updateCart(itemId, restaurantId, 1, price);   // ✅ pass price
    	} else if (e.target.closest('.qty-btn')) {
    	    const btn = e.target.closest('.qty-btn');
    	    const control = btn.closest('.qty-control');
    	    const itemId = control.dataset.id;
    	    const restaurantId = control.dataset.restaurant;
    	    const price = control.dataset.price;   // ✅ keep price
    	    ...
    	    if (count <= 0) {
    	        control.parentElement.innerHTML = `
    	            <button type="button" class="add-btn" data-id="${itemId}" data-restaurant="${restaurantId}" data-price="${price}">
    	                <i class="fas fa-plus"></i> Add
    	            </button>
    	        `;
    	        updateCart(itemId, restaurantId, 0, price);  // ✅ pass price
    	    } else {
    	        countSpan.textContent = count;
    	        updateCart(itemId, restaurantId, count, price);  // ✅ pass price
    	    }
    	}
    });
...
//Handle clicks on disabled "Closed" button
document.addEventListener('click', function(e) {
    if (e.target.closest('.add-btn.disabled')) {
        const alertDiv = document.createElement("div");
        alertDiv.className = "alert alert-danger alert-dismissible fade show position-fixed top-0 end-0 m-3";
        alertDiv.role = "alert";
        alertDiv.innerHTML = `
            <strong>Restaurant is closed.</strong> Please try again later.
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        `;
        document.body.appendChild(alertDiv);
